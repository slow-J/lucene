...
import static org.apache.lucene.codecs.lucene90.LuceneTestPostingsFormat.DOC_CODEC;
import static org.apache.lucene.codecs.lucene90.LuceneTestPostingsFormat.MAX_SKIP_LEVELS;
import static org.apache.lucene.codecs.lucene90.LuceneTestPostingsFormat.PAY_CODEC;
import static org.apache.lucene.codecs.lucene90.LuceneTestPostingsFormat.POS_CODEC;
import static org.apache.lucene.codecs.lucene90.LuceneTestPostingsFormat.TERMS_CODEC;
import static org.apache.lucene.codecs.lucene90.LuceneTestPostingsFormat.VERSION_CURRENT;
import static org.apache.lucene.codecs.lucene90.LuceneTestPostingsFormat.VERSION_START;
...
import org.apache.lucene.codecs.lucene90.LuceneTestPostingsFormat.IntBlockTermState;
 ...
 * @lucene.experimental
 */
public final class LuceneTestPostingsReader extends PostingsReaderBase {
  ...
  /** Sole constructor. */
  public LuceneTestPostingsReader(SegmentReadState state) throws IOException {
    ...
    String docName =
        IndexFileNames.segmentFileName(
            state.segmentInfo.name, state.segmentSuffix, LuceneTestPostingsFormat.DOC_EXTENSION);
    try {
      ...
      if (state.fieldInfos.hasProx()) {
        String proxName =
            IndexFileNames.segmentFileName(
                ...
                state.segmentSuffix,
                LuceneTestPostingsFormat.POS_EXTENSION);
        ...
        if (state.fieldInfos.hasPayloads() || state.fieldInfos.hasOffsets()) {
          String payName =
              IndexFileNames.segmentFileName(
                  state.segmentInfo.name,
                  state.segmentSuffix,
                  LuceneTestPostingsFormat.PAY_EXTENSION);
  ...
  final class BlockDocsEnum extends PostingsEnum {
    ...
    final ForUtilNoP forUtilNoP = new ForUtilNoP(new ForUtil());
    ...
    public BlockDocsEnum(FieldInfo fieldInfo) throws IOException {
      this.startDocIn = LuceneTestPostingsReader.this.docIn;
      this.docIn = null;
    ...
    public int freq() throws IOException {
      if (isFreqsRead == false) {
        forUtilNoP.decode(docIn, freqBuffer); // read freqBuffer for this block
        isFreqsRead = true;
      }
    ...
    private void refillDocs() throws IOException {
      ...
      if (left >= BLOCK_SIZE) {
        forUtilNoP.decodeAndPrefixSum(docIn, accum, docBuffer);

        if (indexHasFreq) {
          if (needsFreq) {
            isFreqsRead = false;
          } else {
            forUtilNoP.skip(docIn); // skip over freqBuffer if we don't need them at all
          }
        }
  ...
  final class EverythingEnum extends PostingsEnum {
    ...
    public EverythingEnum(FieldInfo fieldInfo) throws IOException {
      ...
      this.startDocIn = LuceneTestPostingsReader.this.docIn;
      this.docIn = null;
      this.posIn = LuceneTestPostingsReader.this.posIn.clone();
      if (indexHasOffsets || indexHasPayloads) {
        this.payIn = LuceneTestPostingsReader.this.payIn.clone();
      } else {
        this.payIn = null;
      }
      if (indexHasOffsets) {
    ...
    private void refillDocs() throws IOException {
      ...
      if (left >= BLOCK_SIZE) {
        forUtilNoP.decodeAndPrefixSum(docIn, accum, docBuffer);
        forUtilNoP.decode(docIn, freqBuffer);
        blockUpto += BLOCK_SIZE;
      } else if (docFreq == 1) {
        docBuffer[0] = singletonDocID;
        freqBuffer[0] = totalTermFreq;
    ...
    private void refillPositions() throws IOException {
      if (posIn.getFilePointer() == lastPosBlockFP) {
        ...
        forUtilNoP.decode(posIn, posDeltaBuffer);

        if (indexHasPayloads) {
          if (needsPayloads) {
            forUtilNoP.decode(payIn, payloadLengthBuffer);
            ...
            // written
            forUtilNoP.skip(payIn); // skip over lengths
        ...
        if (indexHasOffsets) {
          if (needsOffsets) {
            forUtilNoP.decode(payIn, offsetStartDeltaBuffer);
            forUtilNoP.decode(payIn, offsetLengthBuffer);
          } else {
            // this works, because when writing a vint block we always force the first length to be
            // written
            forUtilNoP.skip(payIn); // skip over starts
            forUtilNoP.skip(payIn); // skip over lengths
          }
        }
      }
    ...
    private void skipPositions() throws IOException {
      ...
      if (toSkip < leftInBlock) {
        ...
        while (toSkip >= BLOCK_SIZE) {
          ...
          forUtilNoP.skip(posIn);

          if (indexHasPayloads) {
            // Skip payloadLength block:
            forUtilNoP.skip(payIn);
  ...
  final class BlockImpactsDocsEnum extends ImpactsEnum {
    ...
    public BlockImpactsDocsEnum(FieldInfo fieldInfo, IntBlockTermState termState)
      ...
      this.docIn = LuceneTestPostingsReader.this.docIn.clone();
  ...
  final class BlockImpactsEverythingEnum extends ImpactsEnum {
      ...
      if (indexHasFreq) {
        if (isFreqsRead == false) { // previous freq block was not read
          ...
          if (indexHasPos && needsPositions && (posDocUpTo < docUpto)) {
            forUtilNoP.decode(docIn, freqBuffer); // load the previous freq block
          } else {
            forUtilNoP.skip(docIn); // skip it
          }
          isFreqsRead = true;
        }
        if (indexHasPos && needsPositions) {
    ...
    public int nextPosition() throws IOException {
      ...
      if (isFreqsRead == false) {
        forUtilNoP.decode(docIn, freqBuffer); // read freqBuffer for this docs block
        isFreqsRead = true;
      }
